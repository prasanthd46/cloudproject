...
export default function ReviewDeptHeads() {
  const { user } = useAuth();
  ...
  if (loading) {
    return (
      <Layout title="Review Department Heads">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="card">
          <div className="card-content pt-6">
            ...
            <p className="text-3xl font-bold mt-2 text-green-600">{completedReviews.length}</p>
          </div>
        </div>
      </div>
                ...
                <tbody className="divide-y divide-border">
                  {pendingReviews.map((review) => (
                    <tr key={review.ReviewID} className="hover:bg-muted/50">
                      ...
                      <td className="px-6 py-4 text-sm text-muted-foreground">{review.DepartmentName}</td>
                ...
                <tbody className="divide-y divide-border">
                  {completedReviews.map((review) => (
                    <tr key={review.ReviewID} className="hover:bg-muted/50">
                      ...
                      <td className="px-6 py-4 text-sm text-right">
                        <button
                          onClick={() => navigate(`/hr/view-review/${review.ReviewID}`)}
        ...
        <div className="card">
          <div className="card-content">
            <p className="text-center text-muted-foreground py-12">
              No department head reviews assigned yet.
            </p>
          </div>
        </div>
      )}
